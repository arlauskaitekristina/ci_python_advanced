...
from ci_python_advancedapp.models import Client, ClientParking, Parking
...
def test_create_client(web_client, db):
    client_data = {
        ...
        "credit_card": "creditka",
        "car_number": "х999хх999",
    }
...
def test_check_in_parking(web_client, db):
    ...
    client_parking = db.session.scalar(
        select(ClientParking).where(ClientParking.id == 2)
    )
...
def test_create_client_with_factory(web_client, db):
    client: type[dict] = factory.build(dict, FACTORY_CLASS=ClientFactory)
...
def test_create_parking_with_factory(web_client, db):
    parking: type[dict] = factory.build(dict, FACTORY_CLASS=ParkingFactory)
